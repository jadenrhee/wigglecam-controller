...
The other diode comes from USB VBUS, so a bench USB port can never be
...
(hardware debounce first stage). The shutter leaves the enclosure, so
it also gets a PESD5V0S1BA TVS at the connector. WS2812B runs its
...
signal. The sensitive geometry (QSPI nested-lane fanout, crystal, USB
pair with its mirror-pad B.Cu jumpers, power entry, battery path, LED
returns, plane fanouts) is laid out with explicit crossing-free
geometry, and the inner layers carry no signal routing at all, so the
planes stay whole under the fast nets. Everything gates through
