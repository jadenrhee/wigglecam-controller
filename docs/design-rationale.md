...
returns, plane fanouts) is generated by script with explicit
crossing-free geometry; Freerouting handles only the genuinely open
low-speed nets, with the inner layers marked unroutable and completed
nets stripped from its input. Everything gates through
