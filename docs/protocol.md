...
any button/encoder event is latched, so the Pi can poll cheaply or
take a GPIO interrupt instead of polling I2C.
...
| 0x02 | TRIGGER | W | 1 = sync pulse only; 2 = flash + sync (flash reaches steady current ~5 ms before the sync edge; degrades to sync only if FLASH_PCT is 0 **or during the 800 ms post-flash cooldown**, but the sync edge always arrives); any other value is ignored. Writes while STATUS bit0 is set are dropped, so poll it before triggering. Back-to-back writes coalesce (last one wins). |
...
| 0x06-07 | VBAT | R | battery voltage, mV, little-endian u16; read low byte first, which snapshots the high byte so the pair is tear-free. A high byte read without its low byte returns the previous snapshot, not the live value |
...
- The PWM pin idles low and the board's 100 kΩ pulldown holds the
  current-sink reference at 0 V through reset/boot, so the flash
  cannot fire until firmware explicitly commands it.
